...
let token = '', current = null, checkpoints = [], arm = [], kick = [], history = [], prediction = null;
let selecting = false, selectedId = null, dirty = false, polling = false, labelSaved = false;
...
async function api(path, options={}) {
  const response = await fetch(path, {credentials:'include', cache:'no-store', ...options, headers:{'X-Video-Lab':'1', ...(options.body && !(options.body instanceof Blob) ? {'Content-Type':'application/json'}:{}), ...options.headers}});
...
async function openProject(id) {
  ...
  current=data; prediction=null; arm=[]; kick=[]; history=[]; dirty=false; selecting=false; $('stage').classList.remove('selecting');
  ...
  if(data.state!=='uploading'){video.src=`${BASE}/media/${id}${data.preview?'':'/source'}`;video.load();}else{video.removeAttribute('src');video.load();}
  ...
  checkpoints=settings?.target?.checkpoints?.map(p=>({...p}))||[];
  $('targetLane').value=settings?.target?.lane_id??''; $('targetConfirmed').checked=!!settings?.target?.confirmed;
  $('stroke').value=settings?.stroke||'freestyle';
  ...
  updateState(); renderEvents(); updateTargetLabel(); await refreshProjects();
}
function updateState() {
  ...
  $('settingsFields').disabled=locked; $('analyzeButton').disabled=locked||!workerOnline||!$('targetConfirmed').checked||!checkpoints.length||!$('targetLane').value;
  $('retargetButton').hidden=!configured||!['complete','failed','cancelled'].includes(current.state);$('retargetButton').disabled=labelSaved;
  ...
  $('targetButton').disabled=locked; $('resetTarget').disabled=locked; $('targetConfirmed').disabled=locked||!checkpoints.length||!$('targetLane').value;
...
act('analyzeButton',async()=>{
  ...
  if(!checkpoints.length||!$('targetConfirmed').checked||!$('targetLane').value)throw Error('레인과 시작 장면의 선수를 선택하고 같은 선수임을 확인하세요.');
  if(checkpoints.some(p=>p.time_sec<start||p.time_sec>=end)||Math.abs(checkpoints[0].time_sec-start)>1/current.fps+.001)throw Error('분석 시작 프레임에서 선수를 다시 선택하세요. 모든 확인점은 분석 구간 안에 있어야 합니다.');
  const settings={stroke:$('stroke').value,start_sec:start,end_sec:end,distance_m:$('distance').value?Number($('distance').value):null,rotation:$('rotation').value,target:{lane_id:Number($('targetLane').value),race_distance_m:50,confirmed:true,checkpoints}};
...
act('retargetButton',async()=>{
  if(dirty||!confirm('기존 모델 결과를 대체하여 다시 분석합니다. 모델 결과를 본 이력은 유지됩니다. 선수 확인점을 수정할까요?'))return;
  const settings=current.settings,at=video.currentTime;
  await post(endpoint('/retarget'));await openProject(current.id);
  checkpoints=settings.target?.checkpoints?.map(p=>({...p}))||[];$('targetLane').value=settings.target?.lane_id??'';
  $('stroke').value=settings.stroke;$('start').value=settings.start_sec;$('end').value=settings.end_sec;
  $('rotation').value=settings.rotation;$('distance').value=settings.distance_m??'';$('targetConfirmed').checked=false;
  seek(at);updateTargetLabel();message('확인이 필요한 장면에서 같은 선수를 다시 클릭하고 재분석하세요.');
});
...
act('reloadMedia',async()=>{if(!current)return;const id=current.id;const data=await api(endpoint(''));if(current?.id!==id)return;if(!current.preview&&data.preview){await openProject(id);return;}current=data;video.load();updateState();});
...
function mediaUnavailable(){
  if(!current)return;
  const text=current.preview?'재생 파일을 불러오지 못했습니다. 「재생 상태 다시 확인」을 누르세요.':workerOnline?'원본 코덱은 재생할 수 없어 연결된 PC에서 호환 영상으로 변환 중입니다. 완료되면 자동으로 전환됩니다.':'이 브라우저에서 원본 코덱을 재생할 수 없습니다. 분석 PC를 연결하면 호환 영상으로 변환됩니다.';
...
act('saveLabel',async()=>{
  ...
  dirty=false;labelSaved=true;updateState();$('saveStatus').textContent=`저장 완료 · ${label.label_mode==='blinded_manual'?'모델 미확인 수기 기록':'모델 확인 가능성이 있는 보정용 기록'}`;message('수기 기록을 저장했습니다.');
...
const reasonLabels={arm_evidence_gap:'팔 관절 관측 공백',leg_evidence_gap:'다리 관절 관측 공백',possible_tracking_or_camera_jump:'추적·카메라 급변 의심',target_not_found:'선택 선수를 찾지 못함',target_ambiguous:'다른 선수와 구분 불확실',target_lost:'선수 재확인 필요',camera_motion_unverified:'카메라 이동·장면 전환 확인 필요',target_identity_incomplete:'선수 추적 공백으로 전체 횟수 보류'};
function renderResults(){
  ...
  const tracking=prediction.result.target_tracking;
  const track=prediction.result.tracks.find(t=>t.lane_id===(current.settings?.target?.lane_id??1)),comparison=prediction.comparison;
  $('trackingSummary').textContent=tracking?`${tracking.lane_id}레인 · 50m 경기 중 ${current.settings.start_sec}–${current.settings.end_sec}초 구간 · 선수 관측 ${(tracking.coverage*100).toFixed(1)}% (정확도 아님). ${tracking.status==='tracked'?'선택 선수의 추적 표시를 재생하며 검수하세요.':'선수 확인이 필요한 공백이 있어 전체 횟수를 확정하지 않습니다. 아래 다시 볼 구간을 확인하세요.'}`:'이전 고정 영역 방식의 결과입니다. 새 영상은 레인·선수 선택 방식으로 분석합니다.';
  $('poseToggle').checked=!!tracking;
  for(const [key,title,field,diagnostic] of [['arm','팔 동작','arm_strokes','merged_arm_candidate_times_sec'],['kick','발차기','kicks','kick_candidate_times_sec']]){
    ...
    const card=textNode('div','','result-card');card.append(textNode('h3',title),textNode('strong',`내 기록 ${ref?.manual_count??'—'} · 모델 ${measured?.available?measured.count:'보류'}`),textNode('p',`관측 후보 ${times.length}개 · ${measured?.available?'선택 구간의 미검증 추정치':`전체 횟수 미확정 (${reasonLabels[measured?.reason]||measured?.reason||'선수 미검출'})`}`));
...
function updateTargetLabel(){
  $('targetLabel').textContent=checkpoints.length?`${$('targetLane').value||'미선택'}레인 · 선수 확인점 ${checkpoints.length}개 · 선택 선수를 따라 추적합니다`:'레인 번호를 고르고 시작 장면의 선수 몸통을 클릭하세요.';
  $('targetButton').textContent=checkpoints.length?'이 장면에서 같은 선수 다시 확인':'시작 장면의 선수 선택';
  $('targetCheckpoints').replaceChildren();
  for(const p of checkpoints){const b=textNode('button',`${p.time_sec.toFixed(3)}초 · 선수 확인점`);b.onclick=()=>seek(p.time_sec);$('targetCheckpoints').append(b);}
  updateState();draw();
}
act('targetButton',()=>{
  if(!$('targetLane').value)throw Error('먼저 분석할 경기 레인을 선택하세요.');
  selecting=!selecting;video.pause();
  if(selecting&&!checkpoints.length)seek(Math.ceil(Number($('start').value)*current.fps)/current.fps);
  $('stage').classList.toggle('selecting',selecting);message(selecting?'이 레인의 선수 몸통을 한 번 클릭하세요. 고정 영역을 드래그하지 않습니다.':'선수 선택을 종료했습니다.');
});
act('resetTarget',()=>{checkpoints=[];$('targetConfirmed').checked=false;updateTargetLabel();});
for(const id of ['targetLane','rotation','stroke','end','distance'])$(id).addEventListener('change',()=>{$('targetConfirmed').checked=false;updateTargetLabel();});
$('start').addEventListener('change',()=>{checkpoints=[];$('targetConfirmed').checked=false;updateTargetLabel();});
$('targetConfirmed').onchange=updateState;
canvas.onpointerdown=e=>{
  if(!selecting||video.seeking||!video.videoWidth)return;
  const rect=canvas.getBoundingClientRect(),b=bounds(),x=e.clientX-rect.left,y=e.clientY-rect.top;
  if(x<b.x||x>b.x+b.width||y<b.y||y>b.y+b.height)return;
  const p=pointer(e),time_sec=video.currentTime;
  if(time_sec<Number($('start').value)||time_sec>=Number($('end').value)){message('분석 구간 안의 장면을 선택하세요.');return;}
  checkpoints=checkpoints.filter(c=>Math.abs(c.time_sec-time_sec)>=1/current.fps);
  if(checkpoints.length>=30){message('확인점은 최대 30개까지 추가할 수 있습니다.');return;}
  checkpoints.push({time_sec,x:p[0],y:p[1]});checkpoints.sort((a,b)=>a.time_sec-b.time_sec);
  $('targetConfirmed').checked=false;selecting=false;$('stage').classList.remove('selecting');updateTargetLabel();
};
function draw(){
  ...
  for(const p of checkpoints){if(Math.abs(p.time_sec-video.currentTime)>.1)continue;ctx.beginPath();ctx.arc(b.x+p.x*b.width,b.y+p.y*b.height,12,0,Math.PI*2);ctx.stroke();ctx.fillStyle='#fff';ctx.font='bold 15px sans-serif';ctx.fillText(`${$('targetLane').value}레인 · 선택 선수`,b.x+p.x*b.width+16,b.y+p.y*b.height);}
...
// Also update in direct lab tabs; the old worker cached private media/206s.
