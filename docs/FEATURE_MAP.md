...
모든 주요 목록은 20·50·100개 표시와 번호 페이지 이동을 지원한다. 사용자·코치·운영 로그·피드백은 전체 또는 필드별 검색을 서버 페이지네이션과 함께 적용한다. 관리자 기능 7개는 데스크톱의 고정 사이드 메뉴와 모바일의 `운영 메뉴` 드로어에서 전환하며, 일반 사용자 대시보드로 이탈하는 홈 링크는 두지 않는다.
