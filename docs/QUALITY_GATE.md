...
2026-07-24 GitHub Actions `30075211320`에서 핵심 94/94, 운영 API 49/49, 역할별 브라우저 28/28을 통과했다. 운영 health는 Alembic `20260723_07`을 반환했다. 공개 `/tutorial`은 랜딩 진입, 개인·코치·비회원 경로, 실제 화면 12개, 기능별 바로가기, 모바일 단일 열과 비활성 영상 분석·워치 직접 연동 오인 문구 방지를 통과했으며 페이지·동작 오류는 0건이었다. `/api/account/insights`는 비로그인 401, 데모 데이터, 기준/추가 기록 변화, 12개월 고정 구간, 영법·풀 분포, 구조화 세트·플랜·사이클·PB와 `authenticated_customer_only` 범위를 통과했다. `/my-data`의 실제 운영 데이터·빈 상태·비동기 로딩과 모바일 화면, 월간 리포트의 인증 초기화 회귀, 대표 홈·온보딩·계정 보안 흐름도 함께 통과했다.
