...
| 영상 분석 v2 오프라인 테스트 | `tests/test_multiswimmer_analysis.py` | 다중 선수 ID 유지, 실제 레인 복원, 원근 레인 분할, RTMPose 좌표 계약, 선수별 독립 카운팅, 타일 중복 제거, 저신뢰 킥 보류, 20개 표본·채점 계약 |
...
11. 테이블·컬럼·인덱스 변경은 Alembic 리비전과 기대 스키마 리비전, `/api/ready` QA를 함께 갱신한다.
...
2026-08-19에는 공통 그리드·flex·카드·패널의 최소 폭과 한국어 줄바꿈 안전망을 보강하고, 월간 성장 리포트의 와이드 본문을 1,540px로 확장했다. 통계 열은 PC에서 340~380px를 확보하고 1,180px 이하에서 1열로 전환해 성장률·연속 출석·숫자와 단위가 한 글자씩 세로로 눌리는 현상을 제거했다. UI QA는 기존 PC/390px 검사에서 2,560·1,440·1,280·1,024·768·390px 여섯 해상도로 확대하고 화면 밖 요소, flex/grid 겹침, 문구 압축, 숫자 지표 줄바꿈과 리포트 열 전환을 실제 좌표로 검사한다. [GitHub Actions `32225886234`](https://github.com/JinHyunjun/swimtech/actions/runs/32225886234)에서 핵심 122개, 운영 API 53개, 브라우저 37개 화면·발견 문제 0건, Postman 27개 요청·44개 assertion을 모두 통과해 P30을 완료했다.

2026-08-20에는 공통 상단 헤더를 문서 흐름에 배치해 마우스 휠과 함께 스크롤되도록 바꾸고, 고정 사이드바는 화면에 남아 있는 헤더 높이만 추종하도록 조정했다. 헤더 폭이 `body`의 사이드바 여백과 공통 `max-width`에 동시에 제한돼 오른쪽이 비던 원인을 제거하고, 데스크톱 사이드바는 232~268px, 서비스·데이터·운영 본문은 작업 영역 안의 최대 1,280px 프레임, 1,100px 이하는 드로어라는 공통 비율을 적용했다. UI QA는 1,100px 전환점까지 포함한 일곱 해상도에서 헤더 좌우 경계·스크롤 좌표·사이드바 상단·프레임 폭과 중심을 실측한다. 공통 헤더·내비게이션은 전용 검사에서 검증하고 일반 콘텐츠 클릭 목록에서는 제외해 DOM 재주입 뒤 오래된 인덱스가 로그아웃을 누르는 오탐도 제거했다. [GitHub Actions `32332683351`](https://github.com/JinHyunjun/swimtech/actions/runs/32332683351)에서 핵심 122개, 운영 API 53개, 브라우저 37개 화면·발견 문제 0건, Postman 27개 요청·44개 assertion을 모두 통과해 P31을 완료했다.

2026-08-21에는 기능을 삭제하지 않고 처음 보이는 선택과 문장 수를 줄이는 단계적 정보 공개 UI를 적용했다. 공통 메뉴는 오늘·기록·플랜·성장 네 핵심 흐름과 세 개 접힘 그룹·전체 검색·모바일 하단 메뉴로 바꾸고, 랜딩 보조 지표, 대시보드 준비도·추천 근거, 플랜 기준·도구·세션 상세, 일지 가져오기, 드릴 원칙·고급 필터를 필요할 때 펼치도록 재구성했다. 월간 리포트는 요약부터 차트까지 한 방향으로 읽는 단일 흐름으로 바꾸고, 훈련 일지의 `기록 추가`는 모바일 첫 화면 상단에 유지했다. 안전·의료·구매 면책은 접지 않고 항상 노출한다. UI QA는 메뉴 구조·검색·모바일 더보기와 각 대표 화면의 실제 펼침·모드 전환·좌표를 추가 검사하며, 사용자 흐름과 분리된 방문 통계 전송은 운영 API 단계에서 별도로 판정한다. [GitHub Actions `32445712441`](https://github.com/JinHyunjun/swimtech/actions/runs/32445712441)에서 핵심 123개, 운영 API 53개, 브라우저 37개 화면·일곱 해상도·발견 문제 0건, Postman 27개 요청·44개 assertion을 모두 통과해 P32를 완료했다.

2026-08-26에는 공개 경로와 분리된 `analysis_v2/`에 대회 영상용 다중 선수 추적·카운팅 기준선을 구성했다. 겹침 타일 포즈를 중복 제거하고 레인 위치·예측 이동·관절 형태로 ID를 유지하며, 각 트랙의 스트로크·완전한 사이클·킥을 독립 계산한다. 검출 순서 변경, 단기 가림, 서로 다른 주기의 두 선수, 평영·접영 동시 팔 병합, 발목 저가시성, 60% 미만 분절 트랙 결과 보류를 8개 합성 회귀 테스트로 확인했다. 로컬 핵심 132개와 [GitHub Actions `32928964066`](https://github.com/JinHyunjun/swimtech/actions/runs/32928964066)의 핵심 132개·Alembic 단일 head가 통과했다. 실제 640×360 대회 영상 스모크에서는 한 프레임의 포즈 후보 5개를 찾았지만 전체 구간 트랙 분절이 남아 정확도 통과로 보지 않았으며, 공개 API·UI·릴리즈 노트·Notion 서비스 설명서는 갱신하지 않았다.

2026-08-28에는 Galaxy Book4 Pro의 CPU·Arc GPU·NPU를 직접 탐색하고 OpenVINO
...
`tests/test_multiswimmer_analysis.py` 33개로 검증했다. 기존 데이터의 폴더 라벨
...
정확도 통과, 공개 기능, 릴리즈 노트, Notion 업데이트로 간주하지 않는다.

같은 날 v0.2 비교에서는 원근 레인 다각형, 레인별 회전 크롭, RTMPose top-down,
물리 레인 ID 재연결, 장면 전환 점수, 시드가 고정된 20개 영상 매니페스트와 독립
정답 채점 계약을 추가했다. 장면 전환 9개 구간을 단일 샷으로 다시 잘라 동일한 715개
프레임을 비교한 결과 프레임당 평균 검출 레인은 `1.868 → 4.897`, 60% 이상 안정
트랙은 `7 → 93`으로 개선됐다. 로컬 오프라인 회귀 테스트 14개가 통과했다. 그러나
독립 이벤트 정답은 아직 0개이고 일부 팔 예측은 명백한 과소 계수 가능성이 있어
...
당시 79개, 전체 핵심 검사는 204개였다. 합성 9조건의 횟수 오차 합은 67→3으로 줄었지만
...
`tests/test_video_workbench.py` 37개를 Core job에 추가해 현재 핵심 검사는 241개다.
모델 다운로드·실영상·DB 없이 합성 runner로 API 계약을 검증한다.

| 기능 | 검증 매핑 |
| --- | --- |
| 로컬 접근·원본 격리·업로드 한도 | Host/Origin/CSRF 차단, UUID 경로, 크기·해독·프리뷰 실패 시 정리 |
| 재생·프레임 이동 | HTTP Range/suffix/416, 원본 보존·H.264 프리뷰 명령·동일 프레임 시간축 |
| 분석 작업 | 영법·구간·거리·ROI 입력, 단일 실행 대기열, 취소·실행 중 삭제 방지 |
| 수기/모델 분리 | 공개 전 결과 비노출, 블라인드 수정 보존, 공개 후 보정용 표시, 재분석 조건 잠금 |
| 비교·DPS | 판독 불가를 0회로 취급하지 않음, 후보와 확정 집계 분리, 단일 검수 비교는 정확도 아님 |
| 독립 검수·모델 개선 | 다른 ROI 합의 차단, 비정상 이벤트·NaN 차단, 종횡비/회전 불변 신호, 캐시 왕복 |

실제 영상 브라우저 QA는 별도 `scripts/qa_video_workbench.py`에 매핑한다.
로컬 URL만 허용하고 해당 실행이 업로드한 프로젝트만 종료 시 삭제한다.
영상 픽셀 표시(오디오만 로드된 경우 실패), ROI 드래그, 버튼/키보드 A·K, 취소,
수기 저장, 모델 결과 공개, 후보 시점 이동, 내보내기, 320~1920px 가로 넘침을 검사한다.
실제 신경망 추론은 로컬 PC에서 실행하며 운영 서비스 스모크나 일반 CI 성공으로 대체하지 않는다.

### 관리자 영법 분석 TEST 연결 (2026-09-09)

`tests/test_admin_video_lab.py` 40개를 핵심 게이트에 추가했다. 전체 281개가 로컬에서 통과했다.
관리자 권한/소유권, 청크 업로드·해시, 작업 전용 토큰·임대·취소·만료, 영상 범위 재생,
블라인드 라벨과 공개 결과 분리를 검증한다. 정기 UI 검사는 관리자 9개 메뉴와 새 실험실의 인증 연결을 검사한다.
실제 운영 영상 업로드·PC 추론·검수 UI는 별도의 opt-in `scripts/qa_admin_video_lab.py`로 검증한다.
상세 실행 조건과 보관 정책은 [관리자 TEST 운영 문서](ADMIN_VIDEO_LAB.md)를 따른다.
QA에서 눌렀다 취소한 이벤트는 정답이 아니며 최종 QA 라벨은 판독 불가·미검증으로 저장한다.
실영상·스크린샷·내보내기 자료는 무시된 `tmp/analysis_v2/workbench-browser/`에만 둔다.

## 이동 카메라 선택 선수·미디어 회귀 (2026-09-11)

P39 도입 당시 핵심 Python 테스트는 305개(관리자 영상 API 53개, 선택 선수 11개 포함)이며
서비스 워커 회귀 3개를 별도로 실행한다. Linux에서 Windows DPAPI와 OpenCV가 없는 경우의
두 광학 흐름/탐색 테스트는 각각 해당 환경 제약으로 skip된다. Windows 로컬 모델 환경에서는 실행한다.
레인 0/7/10의 ID 유지, 검출 순서 변경, 팬·줌, 겹침, 장면 전환, 장기 가림, 명시적 재확인,
추적 공백의 전체 횟수 보류, 구버전 처리기 차단, 재분석 후 블라인드 상태 복구 금지를 검사한다.
`node --test scripts/test_video_service_worker.mjs`는 Range/API 비캐시, 206·실패·비공개 응답
제외, 캐시 용량 실패 처리, 이전 SwimMate 캐시만 정리하는 경계를 검사한다.

로컬 `scripts/qa_admin_video_lab.py --service-worker-check --lane 7 ...`에서 실제 업로드,
H.264 재생, 체크 전 분석 금지, 레인·선수 확정, PC 모델 반환, 라벨·결과·내보내기,
320~1920px 레이아웃과 리소스 오류 0건, 비공개 미디어 캐시 없음까지 확인했다.
이는 실영상의 선수 식별/이벤트 정답 정확도나 운영 로그인 UI E2E 통과를 뜻하지 않는다.
저해상도 방송 90프레임 smoke에서는 연결 후보 20프레임으로 전체 집계가 보류됐다.

## 산출물

2026-09-15 자동 시작 후속 검증: 현재 Python 핵심 318개와 서비스 워커 3개.
관리자 영상 테스트 66개에는 saved-only 신규 승인 금지, 인증 거부와 일시 작업 토큰 401 분리,
기기 승인 보존, 명시적 등록 후 Windows 작업으로 넘기는 절차, 로그 비밀값 제외,
현재 사용자/배터리/무제한 실행 시간 계약과 거부된 기기의 정기 인증 재시도 금지를 포함한다.
실제 Windows 로그온 작업 등록 및 별도 테스트 작업의 비정상 종료 후 정기 재실행도 확인했다.
서버가 기존 기기 갱신을 거부하면 해당 PC 연결 복구는 별도로 승인/등록 상태를 확인해야 한다.
