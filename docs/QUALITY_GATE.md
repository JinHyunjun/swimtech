...
같은 날 수영복 구매 가이드를 Speedo·arena·TYR·Mizuno·Nike Swim의 공식 성인 일반·훈련용 표와 교차 추천으로 확장했다. 운영 UI QA는 여성/남성·브랜드 표 전환, 현재 모델·사이즈·착용감·실측 기반 5개 결과, 레이싱·테크수트 일반표 환산 차단을 실제 DOM 변화로 확인한다. 국가·제품군별 라벨 차이와 공식 상품표 최종 확인 문구도 계약 테스트로 고정하며 배포 후 검증 전까지 완료로 판정하지 않는다.
