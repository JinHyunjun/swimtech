...
현재 핵심 자동 테스트는 단위·계약·Jira 통합 85개이며 Alembic 단일 head 검사도 같은 작업에서 실행한다. `tests/test_swimtech.py`의 Playwright E2E 정의 104개는 과거 로컬 통합 환경용 참고 시나리오이며 필수 품질 게이트의 통과 수에 포함하지 않는다. 실제 로그인 화면과 배포 서비스는 `qa_runner.py`의 44개 API 시나리오와 `qa_ui_crawler.py`의 역할별 25개 화면 검증으로 일괄 확인하고 실행별 리포트를 증적으로 보관한다.
...
| 월간 리포트 | 훈련 일지와 같은 사용자 기준, 평균 거리, 플랜·세트 수행률, 사이클 포함률, 목표 달성률, 공유 링크 | `qa_runner.py` 17, `report.py` 계약 테스트 |
...
2026-07-23 GitHub Actions `29988388370`에서 핵심 85/85, 운영 API 44/44, 역할별 브라우저 25/25를 통과했다. 운영 health는 Alembic `20260723_05`를 반환했다. 반 일정·출석·공지 회귀와 관리자 전용 로그인을 다시 확인했으며, 새 반 수행 분석은 코치 조회 200, 확인된 출석률·기록 완료율 100%, 코치 코드 비연동 개인훈련 비공개, 학생 조회 403, 테스트 클럽 정리 200을 확인했다.
