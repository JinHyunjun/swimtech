...
현재 핵심 자동 테스트는 단위·계약·Jira 통합 91개이며 Alembic 단일 head 검사도 같은 작업에서 실행한다. `tests/test_swimtech.py`의 Playwright E2E 정의 104개는 과거 로컬 통합 환경용 참고 시나리오이며 필수 품질 게이트의 통과 수에 포함하지 않는다. 실제 로그인 화면과 배포 서비스는 `qa_runner.py`의 48개 API 시나리오와 `qa_ui_crawler.py`의 역할별 26개 화면 검증으로 일괄 확인하고 실행별 리포트를 증적으로 보관한다.
...
2026-07-24 GitHub Actions `30026694346`에서 핵심 91/91, 운영 API 48/48, 역할별 브라우저 26/26을 통과했다. 운영 health는 Alembic `20260723_07`을 반환했다. 일반 로그인·비회원 체험은 `/landing`, 관리자는 `/admin`을 반환했고 `/`·`/app` 307, 온보딩 저장·재조회·추천 반영, 프로필 설정 수정 화면과 홈 링크를 확인했다. 개인 데이터 JSON은 31개 기록 섹션을 내보내면서 비밀번호 해시·토큰을 제외했고, 비밀번호가 틀린 내보내기·비밀번호 변경·전체 로그아웃·탈퇴는 모두 401로 거부됐다. 밝은 테마의 온보딩 카드 제목 색상도 계산 스타일과 운영 스크린샷으로 검증했다.
