...
2026-07-24 GitHub Actions `30073137555`에서 핵심 93/93, 운영 API 49/49, 역할별 브라우저 27/27을 통과했다. 운영 health는 Alembic `20260723_07`을 반환했다. `/api/account/insights`는 비로그인 401, 데모 데이터, 기준/추가 기록 변화, 12개월 고정 구간, 영법·풀 분포, 구조화 세트·플랜·사이클·PB와 `authenticated_customer_only` 범위를 통과했다. `/my-data`의 실제 운영 데이터·빈 상태·비동기 로딩과 모바일 화면을 확인했고, 월간 리포트는 인증 초기화 전에 월 이동 버튼을 눌러도 `undefined/NaN` 요청을 만들지 않도록 회귀 검증했다. 기존 대표 홈·온보딩·계정 보안 흐름도 함께 통과했다.
