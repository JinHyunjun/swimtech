...
8. 코치 강습 문서 생성·수정·선택 배포와 템플릿 폴백
9. Jira 설정 환경에서 과제 생성·완료·웹훅
10. 관리자 20/50/100 페이지 크기, 번호 페이지 이동, 페이지 조회 로그와 피드백 작성자
11. 브라우저 콘솔 오류, 실패 API, 모바일 레이아웃
12. `/meta`, `/upload`, `/viewer`가 영상 분석을 노출하지 않는지 확인
