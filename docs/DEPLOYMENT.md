...
QA 계정 전수 감사와 재분류는 `QA Account Classification` workflow에서 실행한다. 기본 `audit`은 변경에 필요한 아이디만 남기고 이메일·이름·닉네임·IP를 제외한 최소정보 `qa_account_audit.json`을 만들며 계정을 바꾸지 않는다. `apply` 또는 `clear`는 감사 결과를 확인한 뒤 명시적으로 입력한 아이디만 변경하고 같은 범위의 보고서를 증적으로 남긴다.
