...
3. 기존 운영 스키마는 Alembic baseline `20260723_01`로 등록되었고 현재 운영 head는 `20260723_05`이다. `20260723_02`는 개인화 온보딩, `20260723_03`은 세트 단위 수행 데이터, `20260723_04`는 클럽·반·범위별 멤버십, `20260723_05`는 반 일정·출석·공지·읽음을 추가한다.
...
4. 온보딩 설정 저장 → `/auth/me` → 대시보드 추천의 풀·수준·목표 반영 → 기존 값 복원
5. 준비도 저장 → 어드바이저 갱신 → 원래 상태 복원
6. 플랜 생성·저장·일지 전송 → 리포트 플랜 수행률 반영
7. 코치 코드 발급 → 학생 연동 → 권한 확인 → 연동 해제
8. 등록 코치의 클럽·반 생성 → 학생 반 코드 참여 → 비권한 반 생성·비등록 코치 승격 거부 → 테스트 클럽 삭제
9. 반 일정 생성 → 학생 출석 저장·본인 조회 제한 → 반 공지·읽음 → 비권한 일정/공지 403 → 테스트 클럽 삭제
10. 코치 반 수행 분석 → 출석률·기록 완료율 → 개인훈련 비공개 → 학생 접근 403 → 테스트 클럽 삭제
11. 코치 강습 문서 생성·수정·선택 배포와 템플릿 폴백
12. Jira 설정 환경에서 과제 생성·완료·웹훅
13. 관리자 20/50/100 페이지 크기, 번호 페이지 이동, 페이지 조회 로그, 피드백 작성자와 클럽·반 운영 지표
14. 브라우저 콘솔 오류, 실패 API, 모바일 레이아웃
15. `/meta`, `/upload`, `/viewer`가 영상 분석을 노출하지 않는지 확인
