...
두 위치에 함께 유지하는 공통 `style.css`는 시맨틱 레이아웃과 카드·패널·그리드 자식에 `min-width: 0`을 적용해 긴 콘텐츠가 부모 열을 밀어내는 기본 CSS 동작을 차단한다. 제목은 한국어 어절을 우선 보존하고 긴 텍스트는 컨테이너 안에서 강제 탈출 없이 줄바꿈하며, 수치 지표는 tabular 숫자를 사용한다. 월간 리포트처럼 지표 밀도가 높은 화면은 별도 전환점과 숫자 `nowrap`을 함께 두고 브라우저 QA가 2,560·1,440·1,280·1,024·768·390px의 실제 좌표를 검사한다.
