...
로그인 화면은 아이디·비밀번호와 제출 버튼을 하나의 시맨틱 `<form>`으로 묶고 `submit` 이벤트에서 `/auth/login`을 호출한다. 따라서 입력칸에서 Enter를 누르는 흐름과 버튼 클릭, 브라우저 비밀번호 관리자 자동완성이 동일한 로그인 함수·중복 제출 방지·역할별 이동 로직을 사용한다.
...
두 위치에 함께 유지하는 공통 `style.css`는 시맨틱 레이아웃과 카드·패널·그리드 자식에 `min-width: 0`을 적용해 긴 콘텐츠가 부모 열을 밀어내는 기본 CSS 동작을 차단한다. 제목은 한국어 어절을 우선 보존하고 긴 텍스트는 컨테이너 안에서 강제 탈출 없이 줄바꿈하며, 수치 지표는 tabular 숫자를 사용한다. 서비스·데이터·운영 화면의 본문은 사이드바를 제외한 작업 영역 안에서 최대 1,280px 공통 프레임으로 정렬하고, 좁아지면 사용 가능한 폭을 모두 사용한다. 월간 리포트처럼 지표 밀도가 높은 화면은 별도 전환점과 숫자 `nowrap`을 함께 두고 브라우저 QA가 2,560·1,440·1,280·1,100·1,024·768·390px의 실제 좌표를 검사한다.

공통 내비게이션은 현재 URL과 쿼리를 정규화해 활성 링크를 표시하고 `/auth/me`가 성공하면 회원·체험 상태를 갱신하되, 공개 정보 화면의 비로그인 401은 페이지 이동으로 바꾸지 않는다. 데스크톱 사이드바 폭은 화면에 따라 232~268px로 조정하고 1,100px 이하에서는 동일 메뉴를 공통 헤더의 버튼으로 여는 오버레이 드로어로 전환하며 `aria-expanded`·`aria-hidden`·`inert`, ESC 닫기와 본문 스크롤 잠금을 함께 관리한다. 공통 헤더는 `sticky`가 아닌 문서 흐름에 두어 휠 스크롤과 함께 사라지고, 고정 사이드바의 상단 좌표만 현재 보이는 헤더 높이에 맞춰 70px에서 0px까지 동기화한다. `favicon.svg`는 작은 크기에서도 구분되는 수영 선수·물결 도형이며 `theme.js`가 Vercel 정적 자산을 브라우저 탭에 연결하고 manifest가 PWA 아이콘으로 선언한다. FastAPI의 아이콘 호환 라우트도 해당 경로가 사용되는 배포 구성에서는 SVG를 `image/svg+xml`로 응답한다.
