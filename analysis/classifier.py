...
def generate_rule_based_feedback(summary, stroke_type: str, purpose: str = "",
    ...
    if l_avg:
        if ideal_e_min <= l_avg <= ideal_e_max:
            strengths.append({
                ...
                "summary": "왼팔 팔꿈치 각도가 딱 맞아요! 팔꿈치 자세가 아주 좋아요 👍",
            ...
            improvements.append({
                ...
                "comment": f"팔꿈치가 {_severity(diff)} 좁게 굽혀져 있어요. 팔꿈치를 조금 더 구부려주세요.",
                "reason": std["elbow_reason"],
            ...
            improvements.append({
                ...
                "how": "팔꿈치를 안쪽으로 모으면서 손바닥이 뒤쪽을 향하도록 해보세요.",
                "comment": f"팔꿈치가 {_severity(diff)} 넓게 벌어져 있어요. 팔꿈치를 안쪽으로 모아주세요.",
                "reason": std["elbow_reason"],
    ...
    if sym >= sym_good:
        ...
        improvements.append({
            ...
            "comment": f"양팔 움직임이 {_severity(arm_diff)} 차이가 나요. 한 팔씩 번갈아 연습해보세요.",
            "reason": std["symmetry_reason"],
        ...
        improvements.append({
            ...
            "why": _why(SYM_WHY),
            "how": "한 팔씩 번갈아 단팔 드릴을 해보면서 각 팔의 감각을 맞춰보세요.",
            "comment": f"양팔 움직임이 많이 달라요. 한 팔씩 집중해서 연습해보세요.",
            "reason": std["symmetry_reason"],
            "worst_moment": "영상 전반에 걸쳐 좌우 차이가 관찰됩니다.",
            "youtube_url": _build_yt_url(stroke_type, "symmetry", std["youtube_queries"][0]),
        })
    ...
    if freq:
        if ideal_k_min <= freq <= ideal_k_max:
            ...
            improvements.append({
                ...
                "comment": "발차기가 조금 많은 편이에요. 팔 동작에 집중하면서 발차기를 줄여보세요.",
                "reason": std["kick_reason"],
    ...
    if head:
        if ideal_h_min <= head <= ideal_h_max:
            ...
            improvements.append({
                ...
                "how": "물 아래를 바라보듯 턱을 살짝 당겨보세요.",
                "comment": f"머리가 {_severity(h_diff)} 들려 있어요. 시선을 물 아래쪽으로 향해보세요.",
                "reason": std["head_reason"],
