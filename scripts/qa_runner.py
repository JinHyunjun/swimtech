...
하는 일:
  ...
  3) 17개 시나리오 순차 검증 → PASS/FAIL 표 + 종료코드
...
BASE = "https://swimtech.vercel.app"
...
def main():
    ...
    # stats/streak도 같이
    _y = time.strftime("%Y"); _m = int(time.strftime("%m"))
    rs = sess.get(f"{BASE}/api/training-log/stats?year={_y}&month={_m}", timeout=60)
    ...
    if log_id:
        ru = sess.put(f"{BASE}/api/training-log/{log_id}", json={
            ...
            "duration_minutes": 70, "intensity": "높음", "memo": "QA 수정"}, timeout=60)
    ...
    rec(15, "플랜→훈련일지 추가", r.status_code in (200, 201), f"{r.status_code}")

    # ── 16. 리포트/대시보드 ─────────────────────────────
    print("\n[16] 리포트")
    ym = time.strftime("%Y-%m").split("-")
    r = sess.get(f"{BASE}/api/report/monthly?year={ym[0]}&month={int(ym[1])}", timeout=60)
    rec(16, "월간 리포트", r.status_code == 200, f"{r.status_code}")

    # ── 17. 모바일(정적이라 동일) — User-Agent만 모바일로 ─
    print("\n[17] 모바일 응답")
    rr = requests.get(f"{BASE}/landing", timeout=60,
                      headers={"User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X)"})
    rec(17, "모바일 UA 랜딩 응답", rr.status_code == 200,
