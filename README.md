...
- 장비 가이드의 수영복 구매 탭에서 입문·정기 훈련·대회 목적별 핏·소재·관리 기준과 Speedo·arena·TYR·Mizuno·Nike Swim 공식 성인 사이즈표를 비교할 수 있습니다. 현재 모델·사이즈·라벨 국가·착용감, 구매 예정 국가와 선택 신체 치수를 입력하면 각 공식표의 기준 지역을 밝힌 일반·훈련용 참고 후보를 안내합니다. 결과는 구매·착용을 보장하지 않으며 모델별 원단·신축성·압박·컷·안감과 국가별 라벨이 다르므로 실제 품번의 상품표와 교환 조건을 우선해야 합니다. 모델 전용 표가 필요한 레이싱·테크수트는 일반표 환산에서 제외합니다.
