...
- 전용 QA 계정은 DB 표식으로 관리합니다. 일반 운영 로그에서는 QA의 로그인·페이지 조회를 제외하고, `QA 검증 로그`에서 등록 계정·30일 이벤트·페이지 조회·최근 활동과 상세 이력을 따로 검색합니다.
- 사용자·코치·일반 운영 로그·QA 검증 로그·피드백은 검색 항목을 선택해 서버에서 필터링할 수 있으며, 모든 주요 목록은 20/50/100개 표시와 번호 기반 페이지 이동을 지원합니다.
...
- `tests/postman/SwimMate.postman_collection.json` — 공개 health·비로그인 경계, 일반 사용자 쿠키 로그인과 스크린샷 확인 토큰 경계·일지→통계·리포트·익명 결과 카드 생성/조회/종료·내 데이터→삭제, 관리자 방문 그래프·카테고리 검색·일반/QA 로그 분리를 25개 요청으로 실행하는 Postman 스모크
...
현재 소스 기준 핵심 122개를 로컬에서 통과했습니다. 배포 기준 GitHub Actions [`31566609900`](https://github.com/JinHyunjun/swimtech/actions/runs/31566609900)에서는 핵심 121개, 운영 API 53개, 일반·데모·관리자 35개 화면과 공개 결과·클럽 화면 2개를 합친 브라우저 37개, Postman 25개 요청·40개 assertion이 모두 성공했습니다. 새 로그인 폼의 Enter 제출은 계약 테스트와 운영 브라우저 로그인의 필수 경로로 추가했으며, 배포 후 전체 품질 게이트에서 최종 판정합니다.
