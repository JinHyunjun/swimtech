...
- **맞춤 훈련 설정**
  - 첫 로그인에서 현재 수준, 주요 목표, 주당 훈련 횟수, 주로 이용하는 25m/50m 풀을 설정합니다.
...
- 단위·계약·Jira 통합 테스트 87개
- `main` Push·PR에서는 핵심 테스트를 실행합니다.
- 매일 09:00 KST 및 수동 실행에서는 핵심 테스트에 이어 운영 API와 Chromium UI 검사를 일괄 실행합니다.
- `scripts/qa_runner.py` — 인증, 훈련 기록·테스트 세트·코스별 PB·리포트, 코치 코드, 클럽·반 역할·일정·출석·공지·수행 분석, 관리자 API 등 운영 시나리오 46개를 실제 사용자 흐름으로 점검
- `scripts/qa_ui_crawler.py` — 역할별 주요 25개 페이지의 DOM, 콘솔 오류, 링크와 관리자 화면 매핑을 점검
