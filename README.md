...
- **슈퍼 관리자** — 사용자, 일반 사용자 운영 로그와 별도 QA 검증 로그, 피드백 작성자, 훈련 운영 상태와 30일 AI 스크린샷 등록 건수, 준비도, 클럽·반·일정·출석률·공지, 코치 등록·선택 인증과 AI 강습 배포 현황을 조회합니다.
...
- 단위·계약·지식 검색·Jira 통합·Postman 자산 계약 테스트 118개
- `main` Push·PR에서는 핵심 테스트를 실행합니다.
- 매일 09:00 KST 및 수동 실행에서는 핵심 테스트에 이어 운영 API와 Chromium UI 검사를 일괄 실행합니다.
- `scripts/qa_runner.py` — 인증·대표 홈 리다이렉트·AI 지식 근거와 본인 기록 개인화·개인 데이터 내보내기·장기 인사이트·계정 보안, 운동 스크린샷 확인 토큰, 훈련 기록·테스트 세트·코스별 PB·리포트, 코치 코드, 클럽·반 역할·일정·출석·공지·수행 분석, 관리자 API 등 운영 시나리오 51개를 실제 사용자 흐름으로 점검
- `scripts/qa_ui_crawler.py` — 로그인 목적지, 공통 헤더의 홈·프로필·로그아웃·테마 순서, 수영 아이콘, 온보딩 수정·계산 스타일, 목적별 사용 가이드 6개 화면과 개인 데이터의 기록 있음·빈 상태를 포함한 역할별 주요 35개 페이지의 DOM·PC/390px 겹침·콘솔 오류·실패 API와 관리자 화면 매핑을 점검
- `tests/postman/SwimMate.postman_collection.json` — 공개 health·비로그인 경계, 일반 사용자 쿠키 로그인과 스크린샷 확인 토큰 경계·일지→통계·리포트·내 데이터→삭제, 관리자 방문 그래프·카테고리 검색·일반/QA 로그 분리를 22개 요청으로 실행하는 Postman 스모크
...
현재 소스 기준 핵심 118개가 로컬과 GitHub Actions [`31298212335`](https://github.com/JinHyunjun/swimtech/actions/runs/31298212335)에서 통과했습니다. 같은 수동 품질 게이트에서 GitHub Secrets 기반 운영 API 51개, 일반·데모·관리자 포함 브라우저 35개 화면, Postman 요청 22개·assertion 37개도 오류 0건으로 성공했습니다. 공통 헤더의 `/landing` 홈, `/profile` 수정, 로그아웃, 테마 전환, 모바일 메뉴와 일반/QA 운영 로그 분리를 함께 확인했습니다.
