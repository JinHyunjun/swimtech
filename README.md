...
- `analysis_v2/`에는 원근 레인 ROI, 레인별 회전 크롭, RTMPose top-down을 사용해 대회 영상의 여러 선수를 실제 레인 ID로 추적하는 오프라인 v0.2 기준선을 구성했습니다. 영법별 5개씩 총 20개 고정 장면 비교에서 평균 검출 레인은 `1.868 → 4.897`로 늘었지만 독립 이벤트 정답이 없어 카운트 정확도는 아직 미검증입니다. 실제 영상 정확도 게이트 전까지 공개 API·UI에는 연결하지 않습니다. 권리·비동기 인프라 게이트도 별도로 통과해야 합니다.
...
- 단위·계약·지식 검색·Jira 통합·Postman 자산 계약·오프라인 다중 선수 분석 테스트 147개
