...
현재 소스 기준 핵심 123개를 로컬에서 통과했습니다. 배포 기준 최근 완료된 GitHub Actions [`32332683351`](https://github.com/JinHyunjun/swimtech/actions/runs/32332683351)에서는 개편 직전 핵심 122개, 운영 API 53개, 일반·데모·관리자 35개 화면과 공개 결과·클럽 화면 2개를 합친 브라우저 37개, Postman 27개 요청·44개 assertion이 모두 성공했습니다. 새 정보 구조는 로컬 Chromium에서 네 개 핵심 메뉴·세 개 상세 그룹·검색·모바일 하단 메뉴와 대표 화면의 단계적 펼침 동작을 추가 검증했으며, 배포 후 동일 품질 게이트의 최종 판정을 진행합니다. 브라우저 검사는 각 화면을 2,560·1,440·1,280·1,100·1,024·768·390px로 재렌더링해 헤더 전체 폭·스크롤 이동, 사이드바 제외 작업 영역의 최대 1,280px 프레임 정렬, 가로 넘침·요소 겹침·비정상 줄바꿈 0건을 확인합니다.
