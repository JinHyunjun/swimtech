...
- 로그인 직후 `/landing`에서는 이번 주·이번 달·누적 훈련량과 최근 기록, 다음 추천 세션만 먼저 보여주며 전체 기능은 카테고리형 사이드바에서 선택합니다. 같은 메뉴는 주요 기능 페이지에도 계속 유지되어 홈으로 돌아오지 않고 화면 사이를 이동할 수 있고, 1,100px 이하에서는 메뉴 버튼으로 드로어를 엽니다. 서비스 상단은 어느 기능에서나 `SwimMate 홈`·`프로필 수정`·`로그아웃`·`테마 변경` 순서의 공통 헤더를 사용하고 본문과 함께 자연스럽게 스크롤되며, 브라우저 탭과 PWA에는 수영 선수·물결 아이콘을 표시합니다.
...
- **훈련 대시보드**
  - 대표 홈에서는 이번 주 거리·훈련 횟수, 이번 달·누적 거리, 주간 목표, 최근 기록 3개와 다음 추천 세션을 간결하게 확인합니다.
...
- 단위·계약·지식 검색·Jira 통합·Postman 자산 계약 테스트 122개
...
현재 소스 기준 핵심 122개를 로컬에서 통과했습니다. 배포 기준 GitHub Actions [`32332683351`](https://github.com/JinHyunjun/swimtech/actions/runs/32332683351)에서도 핵심 122개, 운영 API 53개, 일반·데모·관리자 35개 화면과 공개 결과·클럽 화면 2개를 합친 브라우저 37개, Postman 27개 요청·44개 assertion이 모두 성공했습니다. 브라우저 검사는 각 화면을 2,560·1,440·1,280·1,100·1,024·768·390px로 재렌더링해 헤더 전체 폭·스크롤 이동, 사이드바 제외 작업 영역의 최대 1,280px 프레임 정렬, 가로 넘침·요소 겹침·비정상 줄바꿈 0건을 확인합니다. 일반·학생·관리자 로그인은 Enter 제출로 역할별 이동까지 확인하며, 운영 감사에서는 확정 QA 4개·미분류 후보 0개와 일반/QA 로그 상호 배제를 확인합니다.
