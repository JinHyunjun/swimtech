...
- 단위·계약·지식 검색·Jira 통합·Postman 자산 계약·오프라인 다중 선수 분석 테스트 132개
...
- `tests/postman/SwimMate.postman_collection.json` — 공개 health·비로그인 경계, 일반 사용자 쿠키 로그인과 스크린샷 확인 토큰 경계·일지→통계·리포트·익명 결과 카드 생성/조회/종료·내 데이터→삭제, 관리자 방문 그래프·QA 계정 후보·자동화 표식·일반/QA 로그 분리를 27개 요청으로 실행하는 Postman 스모크
- `docs/QUALITY_GATE.md` — 기능 유형별로 반드시 넘어가야 할 검증 기준과 증적 산출물을 정리

`tests/test_swimtech.py`의 Playwright 108개 정의는 과거 로컬 통합 환경용 참고 시나리오로 남겨 두었으며 현재 필수 품질 게이트의 통과 수에는 포함하지 않습니다. 배포 검증 증적은 실행마다 생성되는 핵심 HTML·JUnit 리포트, `qa_report.json`, `qa_ui_report.json`, UI 스크린샷으로 판정합니다.
...
현재 소스 기준 핵심 132개를 로컬에서 통과했고 GitHub Actions [`32928964066`](https://github.com/JinHyunjun/swimtech/actions/runs/32928964066)에서도 핵심 132개와 Alembic 단일 head 검사가 성공했습니다. 직전 전체 운영 검증 증적인 [`32445712441`](https://github.com/JinHyunjun/swimtech/actions/runs/32445712441)에서는 추가 전 핵심 123개, 운영 API 53개, 일반·데모·관리자 35개 화면과 공개 결과·클럽 화면 2개를 합친 브라우저 37개, Postman 27개 요청·44개 assertion이 모두 성공했습니다. 브라우저 검사는 네 개 핵심 메뉴·세 개 상세 그룹·검색·모바일 하단 메뉴와 대표 화면의 단계적 펼침을 실제 조작하고, 각 화면을 2,560·1,440·1,280·1,100·1,024·768·390px로 재렌더링해 헤더 전체 폭·스크롤 이동, 사이드바 제외 작업 영역의 최대 1,280px 프레임 정렬, 가로 넘침·요소 겹침·비정상 줄바꿈 0건을 확인합니다.
