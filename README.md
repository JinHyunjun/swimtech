...
- **훈련 일지**
  ...
  - 사용자가 사진첩에서 직접 고른 Apple Fitness 수영 요약 스크린샷 1장을 Gemini가 읽어 날짜·거리·시간·풀 길이·칼로리·심박·랩·영법별 거리를 초안으로 채웁니다. 원본 이미지는 저장하지 않으며, 사용자가 모든 값을 확인·수정하고 확정해야만 일지와 세부 세트가 생성됩니다.
  ...
  - Apple Watch·Galaxy Watch 직접 동기화와 건강 앱 전체 내보내기 파일 가져오기는 여전히 제공하지 않습니다. 스크린샷 등록은 사용자가 선택한 이미지 한 장을 확인 후 기록으로 바꾸는 별도 기능입니다.
- **풀사이드 훈련 실행**
...
`tests/test_swimtech.py`의 Playwright 107개 정의는 과거 로컬 통합 환경용 참고 시나리오로 남겨 두었으며 현재 필수 품질 게이트의 통과 수에는 포함하지 않습니다. 배포 검증 증적은 실행마다 생성되는 핵심 HTML·JUnit 리포트, `qa_report.json`, `qa_ui_report.json`, UI 스크린샷으로 판정합니다.
