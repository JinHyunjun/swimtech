...
- **계정과 개인 데이터 관리**
  ...
  - 프로필·개인화·일지·세트·테스트 기록·PB·플랜·뱃지·챌린지 등 본인 데이터를 JSON 파일로 내보냅니다.
...
- 단위·계약·Jira 통합 테스트 91개
- `main` Push·PR에서는 핵심 테스트를 실행합니다.
- 매일 09:00 KST 및 수동 실행에서는 핵심 테스트에 이어 운영 API와 Chromium UI 검사를 일괄 실행합니다.
- `scripts/qa_runner.py` — 인증·대표 홈 리다이렉트·개인 데이터 내보내기·계정 보안, 훈련 기록·테스트 세트·코스별 PB·리포트, 코치 코드, 클럽·반 역할·일정·출석·공지·수행 분석, 관리자 API 등 운영 시나리오 48개를 실제 사용자 흐름으로 점검
- `scripts/qa_ui_crawler.py` — 로그인 목적지, 홈 링크, 온보딩 수정·계산 스타일, 역할별 주요 26개 페이지의 DOM·콘솔 오류·실패 API와 관리자 화면 매핑을 점검
...
최근 운영 검증은 GitHub Actions `30026694346`에서 핵심 91/91, 운영 API 48/48, 일반·코치·학생·관리자 브라우저 26/26을 통과했습니다.
