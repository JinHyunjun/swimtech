...
- 기능이 많아 어디서 시작할지 모르겠다면 [스크린샷 기능 가이드](https://swimtech.vercel.app/tutorial)에서 개인 훈련과 코치·클럽 흐름을 순서대로 확인할 수 있습니다.
...
- **스크린샷 기능 가이드** — `/tutorial`에서 맞춤 설정 → 준비도 → 플랜 → 풀사이드 실행 → 일지 → 월간·장기 데이터와 코치·클럽 운영의 차이를 실제 화면으로 설명합니다.
...
![SwimMate 스크린샷 기능 가이드](./docs/screenshots/swimmate-feature-tutorial.png)
...
`tests/test_swimtech.py`의 Playwright 106개 정의는 과거 로컬 통합 환경용 참고 시나리오로 남겨 두었으며 현재 필수 품질 게이트의 통과 수에는 포함하지 않습니다. 배포 검증 증적은 실행마다 생성되는 핵심 HTML·JUnit 리포트, `qa_report.json`, `qa_ui_report.json`, UI 스크린샷으로 판정합니다.
...
직전 기준 핵심 109개는 로컬과 GitHub Actions [`31145322123`](https://github.com/JinHyunjun/swimtech/actions/runs/31145322123)에서 통과했습니다. 현재 115개 정의에는 운동 스크린샷의 파일 서명·연도 확인·거리 일관성·사용자별 확인 토큰·확정 트랜잭션·원본 비저장 계약이 추가됐습니다. 인증 Secrets가 필요한 운영 API 51개, 역할별 브라우저 28개와 Postman 요청 20개 전체 일괄 검사는 정기·수동 품질 게이트에서 판정합니다.
