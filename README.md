...
- 로그인 직후 `/landing`에서는 이번 주·이번 달·누적 훈련량과 최근 기록, 다음 추천 세션만 먼저 보여주며 전체 기능은 카테고리형 사이드바에서 선택합니다. 같은 메뉴는 주요 기능 페이지에도 계속 유지되어 홈으로 돌아오지 않고 화면 사이를 이동할 수 있고, 모바일에서는 메뉴 버튼으로 드로어를 엽니다. 서비스 상단은 어느 기능에서나 `SwimMate 홈`·`프로필 수정`·`로그아웃`·`테마 변경` 순서의 공통 헤더를 사용하며 브라우저 탭과 PWA에는 수영 선수·물결 아이콘을 표시합니다.
...
- `scripts/qa_ui_crawler.py` — 로그인 목적지, 공통 헤더의 홈·프로필·로그아웃·테마 순서, 수영 아이콘, 역할별 주요 35개 페이지와 고정 fixture 기반 공개 결과 카드·클럽 초대 2개 화면의 DOM을 점검합니다. 2,560·1,440·1,280·1,024·768·390px에서 가로 넘침·요소 겹침·비정상 줄바꿈을 검사하고 결과 PNG·초대 경로·콘솔 오류도 함께 확인합니다.
