...
- **슈퍼 관리자** — 사용자, 페이지 조회·운영 로그, 피드백 작성자, 훈련 운영 상태와 30일 AI 스크린샷 등록 건수, 준비도, 클럽·반·일정·출석률·공지, 코치 등록·선택 인증과 AI 강습 배포 현황을 조회합니다.
- 사용자·코치·운영 로그·피드백은 검색 항목을 선택해 서버에서 필터링할 수 있으며, 모든 주요 목록은 20/50/100개 표시와 번호 기반 페이지 이동을 지원합니다.
...
- 단위·계약·지식 검색·Jira 통합·Postman 자산 계약 테스트 115개
...
- `tests/postman/SwimMate.postman_collection.json` — 공개 health·비로그인 경계, 일반 사용자 쿠키 로그인과 스크린샷 확인 토큰 경계·일지→통계·리포트·내 데이터→삭제, 관리자 방문 그래프 데이터·카테고리 검색·페이지네이션을 20개 요청으로 실행하는 Postman 스모크
...
직전 기준 핵심 115개는 로컬과 GitHub Actions [`31244897257`](https://github.com/JinHyunjun/swimtech/actions/runs/31244897257)에서 통과했습니다. 현재 운영 UI 정의는 사용 가이드 허브와 5개 상세 페이지를 포함한 역할별 브라우저 33개 화면이며, 인증 Secrets가 필요한 운영 API 51개와 Postman 요청 20개 전체 일괄 검사는 정기·수동 품질 게이트에서 판정합니다.
