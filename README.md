...
- 로그인 직후 `/landing`에서는 이번 주·이번 달·누적 훈련량과 최근 기록, 다음 추천 세션만 먼저 보여주며 전체 기능은 카테고리형 사이드바에서 선택합니다. 모바일에서는 왼쪽 위 메뉴 버튼으로 같은 사이드바를 엽니다.
...
- 단위·계약·지식 검색·Jira 통합·Postman 자산 계약 테스트 108개
...
현재 소스 기준 핵심 108개는 로컬과 GitHub Actions [`31143808467`](https://github.com/JinHyunjun/swimtech/actions/runs/31143808467)에서 통과했습니다. 대표 홈은 운영 비회원 체험에서 네 대시보드 API의 200 응답, 실제 훈련량·최근 기록·추천 렌더링, 데스크톱·390px 사이드바와 오류 부재를 확인했습니다. 인증 Secrets가 필요한 운영 API 50개, 역할별 브라우저 28개와 Postman 요청 19개 전체 일괄 검사는 정기·수동 품질 게이트에서 판정합니다.
